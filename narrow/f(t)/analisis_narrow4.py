...
for i in range(0,1):
  ...
  while i<len(vx)/3:
      ...
      locals()['f_neta_central_vd{0}'.format(v)]+=[((v-vx[i*3])*m/tau)+f_social[i*3]-f_granular[i*3]] # F neta part central(t)
      i+=1
...
plt.semilogy(t_central_vd2,f_neta_central_vd2,'k',label='Net',lw=0.7,zorder=2)
plt.semilogy(t_central_vd2,f_granular_central,'g',label='friction',lw=0.7,zorder=2)
plt.semilogy(t_central_vd2,f_social_central,'r',label='social',lw=0.7,zorder=2)
plt.semilogy(t_central_vd2,f_desired_central,'b',label='desired',lw=0.7,zorder=2)
...
pylab.savefig('f(t)_vd2_log.eps', format='eps', dpi=300, bbox_inches='tight')